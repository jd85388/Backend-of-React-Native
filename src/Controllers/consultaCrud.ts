...
export const crearConsulta = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const obtenerConsultas = async (req: Request, res: Response): Promise<void> => {
    try {
        const idPaciente = req.idPaciente;
        if (!idPaciente) {
            res.status(401).json({ mensaje: 'Usuario no autenticado' });
...
export const obtenerConsultasProximas = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const obtenerHistorialConsultas = async (req: Request, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        if (!userId) {
            res.status(401).json({ mensaje: 'Usuario no autenticado' });
...
export const obtenerConsultaPorId = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const idPaciente = req.idPaciente;
...
// Actualizar consulta
export const actualizarConsulta = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const idPaciente = req.idPaciente;
...
// Eliminar consulta
export const eliminarConsulta = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const idPaciente = req.idPaciente;
...
export const cancelarConsulta = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const consultaCancelada = await Consulta.findOneAndUpdate(
            { _id: id, id_Paciente: idPaciente },
            { estado: false },
            { new: true }
        );
