...
class CitaController {
    ...
    async obtenerCitasProximas(req, res) {
        try {
            const { id_paciente } = req.params;
            const diasAdelante = parseInt(req.query.dias) || 7;
            const fechaActual = new Date();
            const fechaLimite = new Date();
            fechaLimite.setDate(fechaActual.getDate() + diasAdelante);
            const citasProximas = await ConsultaService.obtenerConsultasEnRango(id_paciente, fechaActual, fechaLimite);
            res.status(200).json({
                success: true,
    ...
    async obtenerCitasDelDia(req, res) {
        try {
            const { id_paciente } = req.params;
            ...
            const citasDelDia = await ConsultaService.obtenerConsultasEnRango(id_paciente, inicioDelDia, finDelDia);
            res.status(200).json({
                success: true,
