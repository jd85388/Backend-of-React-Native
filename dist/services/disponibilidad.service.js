import { Disponibilidad, CitaProgramada } from '../models/disponibilidad';
export class DisponibilidadService {
    ...
    async obtenerMedicosPorEspecialidad(codigoEspecialidad) {
    ...
    async cancelarCita(citaId, motivo) {
        return await CitaProgramada.findByIdAndUpdate(citaId, {
            estado: 'cancelada',
    ...
    // Reprogramar cita
    async reprogramarCita(citaId, nuevaFecha, nuevaHora) {
        const cita = await CitaProgramada.findById(citaId);
        if (!cita) {
            throw new Error('Cita no encontrada');
        }
        // Verificar disponibilidad del nuevo horario
        const horariosDisponibles = await this.obtenerHorariosDisponibles(cita.disponibilidadId.toString(), nuevaFecha);
    ...
    async obtenerAgendaMedico(disponibilidadId, fecha) {
        const fechaSinHora = new Date(fecha);
        fechaSinHora.setHours(0, 0, 0, 0);
